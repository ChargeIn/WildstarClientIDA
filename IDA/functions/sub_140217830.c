#include "../winhttp.h"

//----- (0000000140217830) ----------------------------------------------------
__int64 sub_140217830()
{
	if (qword_140C63838)
		return qword_140C63838(off_140A6B748, qword_140C63858);
	if (dword_140C639F0 || (int)sub_140217620() < 0)
		return 0i64;
	return (*(__int64(__fastcall**)(__int64))(*(_QWORD*)qword_140C64220 + 40i64))(qword_140C64220);
}
// 140A6B748: using guessed type wchar_t *off_140A6B748[2];
// 140C63838: using guessed type __int64 (__fastcall *qword_140C63838)(_QWORD, _QWORD);
// 140C63858: using guessed type __int64 qword_140C63858;
// 140C639F0: using guessed type int dword_140C639F0;
// 140C64220: using guessed type __int64 qword_140C64220;

