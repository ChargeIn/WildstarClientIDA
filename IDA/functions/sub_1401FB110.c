//----- (00000001401FB110) ----------------------------------------------------
__int64 sub_1401FB110()
{
	if (qword_140C63838)
		return qword_140C63838(off_140A69FE0, qword_140C63858);
	if (dword_140C648B8 || (int)sub_1401FAF00() < 0)
		return 0i64;
	return (*(__int64(__fastcall**)(__int64))(*(_QWORD*)qword_140C64FE0 + 40i64))(qword_140C64FE0);
}
// 140A69FE0: using guessed type wchar_t *off_140A69FE0[2];
// 140C63838: using guessed type __int64 (__fastcall *qword_140C63838)(_QWORD, _QWORD);
// 140C63858: using guessed type __int64 qword_140C63858;
// 140C648B8: using guessed type int dword_140C648B8;
// 140C64FE0: using guessed type __int64 qword_140C64FE0;

