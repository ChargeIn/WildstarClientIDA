#include "../winhttp.h"

//----- (00000001401E8790) ----------------------------------------------------
__int64 sub_1401E8790()
{
	if (qword_140C63838)
		return qword_140C63838(off_140A69090, qword_140C63858);
	if (dword_140C64F2C || (int)sub_1401E8580() < 0)
		return 0i64;
	return (*(__int64(__fastcall**)(__int64))(*(_QWORD*)qword_140C64A98 + 40i64))(qword_140C64A98);
}
// 140A69090: using guessed type wchar_t *off_140A69090[2];
// 140C63838: using guessed type __int64 (__fastcall *qword_140C63838)(_QWORD, _QWORD);
// 140C63858: using guessed type __int64 qword_140C63858;
// 140C64A98: using guessed type __int64 qword_140C64A98;
// 140C64F2C: using guessed type int dword_140C64F2C;

