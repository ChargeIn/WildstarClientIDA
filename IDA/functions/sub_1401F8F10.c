#include "../winhttp.h"

//----- (00000001401F8F10) ----------------------------------------------------
__int64 sub_1401F8F10()
{
	if (qword_140C63838)
		return qword_140C63838(off_140A69E20, qword_140C63858);
	if (dword_140C63A24 || (int)sub_1401F8D00() < 0)
		return 0i64;
	return (*(__int64(__fastcall**)(__int64))(*(_QWORD*)qword_140C65098 + 40i64))(qword_140C65098);
}
// 140A69E20: using guessed type wchar_t *off_140A69E20[2];
// 140C63838: using guessed type __int64 (__fastcall *qword_140C63838)(_QWORD, _QWORD);
// 140C63858: using guessed type __int64 qword_140C63858;
// 140C63A24: using guessed type int dword_140C63A24;
// 140C65098: using guessed type __int64 qword_140C65098;

