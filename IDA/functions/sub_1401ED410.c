#include "../winhttp.h"

//----- (00000001401ED410) ----------------------------------------------------
__int64 sub_1401ED410()
{
	if (qword_140C63838)
		return qword_140C63838(off_140A69480, qword_140C63858);
	if (dword_140C64C7C || (int)sub_1401ED200() < 0)
		return 0i64;
	return (*(__int64(__fastcall**)(__int64))(*(_QWORD*)qword_140C63DF8 + 40i64))(qword_140C63DF8);
}
// 140A69480: using guessed type wchar_t *off_140A69480[2];
// 140C63838: using guessed type __int64 (__fastcall *qword_140C63838)(_QWORD, _QWORD);
// 140C63858: using guessed type __int64 qword_140C63858;
// 140C63DF8: using guessed type __int64 qword_140C63DF8;
// 140C64C7C: using guessed type int dword_140C64C7C;

