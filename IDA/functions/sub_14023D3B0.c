#include "../winhttp.h"

//----- (000000014023D3B0) ----------------------------------------------------
__int64 sub_14023D3B0()
{
	if (qword_140C63838)
		return qword_140C63838(off_140A6D658, qword_140C63858);
	if (dword_140C64C94 || (int)sub_14023D1A0() < 0)
		return 0i64;
	return (*(__int64(__fastcall**)(__int64))(*(_QWORD*)qword_140C64FE8 + 40i64))(qword_140C64FE8);
}
// 140A6D658: using guessed type wchar_t *off_140A6D658[2];
// 140C63838: using guessed type __int64 (__fastcall *qword_140C63838)(_QWORD, _QWORD);
// 140C63858: using guessed type __int64 qword_140C63858;
// 140C64C94: using guessed type int dword_140C64C94;
// 140C64FE8: using guessed type __int64 qword_140C64FE8;

