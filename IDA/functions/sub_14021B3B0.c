//----- (000000014021B3B0) ----------------------------------------------------
__int64 sub_14021B3B0()
{
	if (qword_140C63838)
		return qword_140C63838(off_140A6BA58, qword_140C63858);
	if (dword_140C648A0 || (int)sub_14021B1A0() < 0)
		return 0i64;
	return (*(__int64(__fastcall**)(__int64))(*(_QWORD*)qword_140C653B0 + 40i64))(qword_140C653B0);
}
// 140A6BA58: using guessed type wchar_t *off_140A6BA58[2];
// 140C63838: using guessed type __int64 (__fastcall *qword_140C63838)(_QWORD, _QWORD);
// 140C63858: using guessed type __int64 qword_140C63858;
// 140C648A0: using guessed type int dword_140C648A0;
// 140C653B0: using guessed type __int64 qword_140C653B0;

