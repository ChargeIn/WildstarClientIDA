//----- (000000014073FE40) ----------------------------------------------------
void __fastcall sub_14073FE40(__int64 a1, int a2)
{
	int v4; // eax
	int v5[2]; // [rsp+20h] [rbp-49h] BYREF
	__int64 v6; // [rsp+28h] [rbp-41h]
	int v7; // [rsp+30h] [rbp-39h]
	int v8; // [rsp+34h] [rbp-35h]
	int v9; // [rsp+38h] [rbp-31h]
	int v10; // [rsp+3Ch] [rbp-2Dh]
	int v11; // [rsp+40h] [rbp-29h]
	int v12; // [rsp+44h] [rbp-25h]
	int v13; // [rsp+48h] [rbp-21h]
	int v14; // [rsp+4Ch] [rbp-1Dh]
	int v15; // [rsp+50h] [rbp-19h]
	int v16; // [rsp+54h] [rbp-15h]
	int v17; // [rsp+58h] [rbp-11h]
	int v18; // [rsp+5Ch] [rbp-Dh]
	int v19; // [rsp+60h] [rbp-9h]
	int v20; // [rsp+64h] [rbp-5h]
	int v21; // [rsp+68h] [rbp-1h]
	int v22; // [rsp+6Ch] [rbp+3h]
	int v23; // [rsp+70h] [rbp+7h]
	int v24; // [rsp+74h] [rbp+Bh]
	int v25; // [rsp+78h] [rbp+Fh]
	int v26; // [rsp+7Ch] [rbp+13h]
	int v27; // [rsp+80h] [rbp+17h]
	int v28; // [rsp+84h] [rbp+1Bh]
	int v29; // [rsp+88h] [rbp+1Fh]
	int v30; // [rsp+8Ch] [rbp+23h]
	int v31; // [rsp+90h] [rbp+27h]
	int v32; // [rsp+94h] [rbp+2Bh]
	int v33; // [rsp+98h] [rbp+2Fh]
	int v34; // [rsp+9Ch] [rbp+33h]
	int v35; // [rsp+A0h] [rbp+37h]
	int v36; // [rsp+A4h] [rbp+3Bh]

	sub_1407E4830(v5, 0i64, 0x88ui64);
	v5[0] = *(_DWORD*)(a1 + 8);
	v5[1] = *(_DWORD*)(a1 + 12);
	v6 = *(_QWORD*)(a1 + 16);
	v4 = *(_DWORD*)(a1 + 52);
	v36 = a2;
	v35 = v4;
	v7 = *(_DWORD*)(a1 + 24);
	v8 = *(_DWORD*)(a1 + 140);
	v9 = *(_DWORD*)(a1 + 144);
	v10 = *(_DWORD*)(a1 + 148);
	v11 = *(_DWORD*)(a1 + 28);
	v12 = *(_DWORD*)(a1 + 152);
	v13 = *(_DWORD*)(a1 + 156);
	v14 = *(_DWORD*)(a1 + 160);
	v15 = *(_DWORD*)(a1 + 32);
	v16 = *(_DWORD*)(a1 + 164);
	v17 = *(_DWORD*)(a1 + 168);
	v18 = *(_DWORD*)(a1 + 172);
	v19 = *(_DWORD*)(a1 + 36);
	v20 = *(_DWORD*)(a1 + 176);
	v21 = *(_DWORD*)(a1 + 180);
	v22 = *(_DWORD*)(a1 + 184);
	v23 = *(_DWORD*)(a1 + 40);
	v24 = *(_DWORD*)(a1 + 188);
	v25 = *(_DWORD*)(a1 + 192);
	v26 = *(_DWORD*)(a1 + 196);
	v27 = *(_DWORD*)(a1 + 44);
	v28 = *(_DWORD*)(a1 + 200);
	v29 = *(_DWORD*)(a1 + 204);
	v30 = *(_DWORD*)(a1 + 208);
	v31 = *(_DWORD*)(a1 + 48);
	v32 = *(_DWORD*)(a1 + 212);
	v33 = *(_DWORD*)(a1 + 216);
	v34 = *(_DWORD*)(a1 + 220);
	sub_1403F4900(qword_140C65898, 0x255u, (__int64)v5);
}
// 140C65898: using guessed type __int64 qword_140C65898;

