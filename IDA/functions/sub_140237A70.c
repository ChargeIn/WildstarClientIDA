//----- (0000000140237A70) ----------------------------------------------------
__int64 sub_140237A70()
{
	if (qword_140C63838)
		return qword_140C63838(off_140A6D1C0, qword_140C63858);
	if (dword_140C6524C || (int)sub_140237860() < 0)
		return 0i64;
	return (*(__int64(__fastcall**)(__int64))(*(_QWORD*)qword_140C64368 + 40i64))(qword_140C64368);
}
// 140A6D1C0: using guessed type wchar_t *off_140A6D1C0[2];
// 140C63838: using guessed type __int64 (__fastcall *qword_140C63838)(_QWORD, _QWORD);
// 140C63858: using guessed type __int64 qword_140C63858;
// 140C64368: using guessed type __int64 qword_140C64368;
// 140C6524C: using guessed type int dword_140C6524C;

