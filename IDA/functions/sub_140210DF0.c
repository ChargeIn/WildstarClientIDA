//----- (0000000140210DF0) ----------------------------------------------------
__int64 sub_140210DF0()
{
	if (qword_140C63838)
		return qword_140C63838(off_140A6B1D0, qword_140C63858);
	if (dword_140C63DD0 || (int)sub_140210BE0() < 0)
		return 0i64;
	return (*(__int64(__fastcall**)(__int64))(*(_QWORD*)qword_140C64378 + 40i64))(qword_140C64378);
}
// 140A6B1D0: using guessed type wchar_t *off_140A6B1D0[2];
// 140C63838: using guessed type __int64 (__fastcall *qword_140C63838)(_QWORD, _QWORD);
// 140C63858: using guessed type __int64 qword_140C63858;
// 140C63DD0: using guessed type int dword_140C63DD0;
// 140C64378: using guessed type __int64 qword_140C64378;

