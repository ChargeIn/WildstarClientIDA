#include "../winhttp.h"

//----- (000000014053C500) ----------------------------------------------------
__int64 __fastcall sub_14053C500(__int64 a1)
{
	int v2; // eax
	__int64 v3; // rcx
	int* v4; // rax
	int* v5; // rax
	int* v6; // rax
	int* v7; // rax
	int* v8; // rax
	int* v9; // rax
	int* v10; // rax
	int* v11; // rax

	*(_QWORD*)a1 = off_140B6C608;
	v2 = dword_140C1E71C;
	*(_QWORD*)(a1 + 8) = 0i64;
	*(_QWORD*)(a1 + 16) = 0i64;
	*(_QWORD*)(a1 + 24) = 0i64;
	*(_QWORD*)(a1 + 32) = 0i64;
	*(_QWORD*)(a1 + 40) = 0i64;
	*(_QWORD*)(a1 + 48) = 0i64;
	*(_QWORD*)(a1 + 56) = 0i64;
	*(_QWORD*)(a1 + 64) = 0i64;
	*(_QWORD*)(a1 + 72) = 0i64;
	*(_QWORD*)(a1 + 80) = 0i64;
	*(_QWORD*)(a1 + 88) = 7864i64;
	v3 = a1 + 104;
	*(_DWORD*)(v3 - 8) = ++v2;
	*(_QWORD*)(v3 + 104) = 0i64;
	*(_QWORD*)(v3 + 112) = 0i64;
	*(_QWORD*)(v3 + 136) = 0i64;
	*(_QWORD*)(v3 + 144) = 0i64;
	*(_QWORD*)(v3 + 168) = 0i64;
	*(_QWORD*)(v3 + 176) = 0i64;
	dword_140C1E71C = v2;
	sub_1407E4830((int*)v3, 0i64, 0xC8ui64);
	*(_DWORD*)(a1 + 304) = 0;
	*(_QWORD*)(a1 + 312) = 0i64;
	*(_QWORD*)(a1 + 320) = 0i64;
	*(_QWORD*)(a1 + 328) = 0i64;
	*(_QWORD*)(a1 + 336) = 0i64;
	*(_DWORD*)(a1 + 344) = 0;
	*(_OWORD*)(a1 + 352) = 0i64;
	*(_DWORD*)(a1 + 368) = 0;
	*(_OWORD*)(a1 + 384) = 0i64;
	*(_QWORD*)(a1 + 400) = 0i64;
	*(_QWORD*)(a1 + 408) = 0i64;
	*(_DWORD*)(a1 + 416) = 0;
	*(_QWORD*)(a1 + 420) = 1065353216i64;
	*(_DWORD*)(a1 + 428) = 0;
	*(_QWORD*)(a1 + 432) = -1i64;
	*(_QWORD*)(a1 + 448) = 0i64;
	*(_QWORD*)(a1 + 440) = 0i64;
	*(_QWORD*)(a1 + 456) = 0i64;
	*(_QWORD*)(a1 + 472) = 0i64;
	*(_QWORD*)(a1 + 464) = 0i64;
	*(_QWORD*)(a1 + 488) = 0i64;
	*(_QWORD*)(a1 + 480) = 0i64;
	v4 = sub_14018B280(40i64, 0);
	*(_QWORD*)(a1 + 504) = v4;
	*(_QWORD*)(a1 + 512) = 0i64;
	*(_BYTE*)v4 = 0;
	*(_QWORD*)(*(_QWORD*)(a1 + 504) + 8i64) = 0i64;
	*(_QWORD*)(*(_QWORD*)(a1 + 504) + 16i64) = *(_QWORD*)(a1 + 504);
	*(_QWORD*)(*(_QWORD*)(a1 + 504) + 24i64) = *(_QWORD*)(a1 + 504);
	*(_QWORD*)(a1 + 528) = 0i64;
	*(_QWORD*)(a1 + 536) = 0i64;
	*(_QWORD*)(a1 + 544) = 0i64;
	*(_QWORD*)(a1 + 552) = 0i64;
	*(_QWORD*)(a1 + 560) = 0i64;
	*(_QWORD*)(a1 + 568) = 0i64;
	v5 = sub_14018B280(40i64, 0);
	*(_QWORD*)(a1 + 592) = 0i64;
	*(_QWORD*)(a1 + 584) = v5;
	*(_BYTE*)v5 = 0;
	*(_QWORD*)(*(_QWORD*)(a1 + 584) + 8i64) = 0i64;
	*(_QWORD*)(*(_QWORD*)(a1 + 584) + 16i64) = *(_QWORD*)(a1 + 584);
	*(_QWORD*)(*(_QWORD*)(a1 + 584) + 24i64) = *(_QWORD*)(a1 + 584);
	*(_QWORD*)(a1 + 608) = 0i64;
	*(_QWORD*)(a1 + 616) = 0i64;
	*(_QWORD*)(a1 + 624) = 0i64;
	v6 = sub_14018B280(48i64, 0);
	*(_QWORD*)(a1 + 648) = 0i64;
	*(_QWORD*)(a1 + 640) = v6;
	*(_BYTE*)v6 = 0;
	*(_QWORD*)(*(_QWORD*)(a1 + 640) + 8i64) = 0i64;
	*(_QWORD*)(*(_QWORD*)(a1 + 640) + 16i64) = *(_QWORD*)(a1 + 640);
	*(_QWORD*)(*(_QWORD*)(a1 + 640) + 24i64) = *(_QWORD*)(a1 + 640);
	v7 = sub_14018B280(48i64, 0);
	*(_QWORD*)(a1 + 680) = 0i64;
	*(_QWORD*)(a1 + 672) = v7;
	*(_BYTE*)v7 = 0;
	*(_QWORD*)(*(_QWORD*)(a1 + 672) + 8i64) = 0i64;
	*(_QWORD*)(*(_QWORD*)(a1 + 672) + 16i64) = *(_QWORD*)(a1 + 672);
	*(_QWORD*)(*(_QWORD*)(a1 + 672) + 24i64) = *(_QWORD*)(a1 + 672);
	v8 = sub_14018B280(48i64, 0);
	*(_QWORD*)(a1 + 712) = 0i64;
	*(_QWORD*)(a1 + 704) = v8;
	*(_BYTE*)v8 = 0;
	*(_QWORD*)(*(_QWORD*)(a1 + 704) + 8i64) = 0i64;
	*(_QWORD*)(*(_QWORD*)(a1 + 704) + 16i64) = *(_QWORD*)(a1 + 704);
	*(_QWORD*)(*(_QWORD*)(a1 + 704) + 24i64) = *(_QWORD*)(a1 + 704);
	v9 = sub_14018B280(48i64, 0);
	*(_QWORD*)(a1 + 744) = 0i64;
	*(_QWORD*)(a1 + 736) = v9;
	*(_BYTE*)v9 = 0;
	*(_QWORD*)(*(_QWORD*)(a1 + 736) + 8i64) = 0i64;
	*(_QWORD*)(*(_QWORD*)(a1 + 736) + 16i64) = *(_QWORD*)(a1 + 736);
	*(_QWORD*)(*(_QWORD*)(a1 + 736) + 24i64) = *(_QWORD*)(a1 + 736);
	*(_DWORD*)(a1 + 760) = 1;
	*(_QWORD*)(a1 + 784) = 0i64;
	*(_QWORD*)(a1 + 792) = 0i64;
	*(_QWORD*)(a1 + 808) = 0i64;
	*(_QWORD*)(a1 + 816) = 0i64;
	*(_DWORD*)(a1 + 768) = 1414420037;
	*(_QWORD*)(a1 + 836) = 0i64;
	*(_DWORD*)(a1 + 844) = 0;
	v10 = sub_14018B280(64i64, 0);
	*(_QWORD*)(a1 + 864) = 0i64;
	*(_QWORD*)(a1 + 856) = v10;
	*(_BYTE*)v10 = 0;
	*(_QWORD*)(*(_QWORD*)(a1 + 856) + 8i64) = 0i64;
	*(_QWORD*)(*(_QWORD*)(a1 + 856) + 16i64) = *(_QWORD*)(a1 + 856);
	*(_QWORD*)(*(_QWORD*)(a1 + 856) + 24i64) = *(_QWORD*)(a1 + 856);
	v11 = sub_14018B280(72i64, 0);
	*(_QWORD*)(a1 + 896) = 0i64;
	*(_QWORD*)(a1 + 888) = v11;
	*(_BYTE*)v11 = 0;
	*(_QWORD*)(*(_QWORD*)(a1 + 888) + 8i64) = 0i64;
	*(_QWORD*)(*(_QWORD*)(a1 + 888) + 16i64) = *(_QWORD*)(a1 + 888);
	*(_QWORD*)(*(_QWORD*)(a1 + 888) + 24i64) = *(_QWORD*)(a1 + 888);
	return a1;
}
// 140B6C608: using guessed type __int64 (__fastcall *off_140B6C608[2])();
// 140C1E71C: using guessed type int dword_140C1E71C;

