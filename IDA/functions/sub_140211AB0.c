//----- (0000000140211AB0) ----------------------------------------------------
__int64 sub_140211AB0()
{
	if (qword_140C63838)
		return qword_140C63838(off_140A6B278, qword_140C63858);
	if (dword_140C63BE8 || (int)sub_1402118A0() < 0)
		return 0i64;
	return (*(__int64(__fastcall**)(__int64))(*(_QWORD*)qword_140C64F00 + 40i64))(qword_140C64F00);
}
// 140A6B278: using guessed type wchar_t *off_140A6B278[2];
// 140C63838: using guessed type __int64 (__fastcall *qword_140C63838)(_QWORD, _QWORD);
// 140C63858: using guessed type __int64 qword_140C63858;
// 140C63BE8: using guessed type int dword_140C63BE8;
// 140C64F00: using guessed type __int64 qword_140C64F00;

