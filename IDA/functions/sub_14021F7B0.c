//----- (000000014021F7B0) ----------------------------------------------------
__int64 sub_14021F7B0()
{
	if (qword_140C63838)
		return qword_140C63838(off_140A6BDD8, qword_140C63858);
	if (dword_140C64B5C || (int)sub_14021F5A0() < 0)
		return 0i64;
	return (*(__int64(__fastcall**)(__int64))(*(_QWORD*)qword_140C64A70 + 40i64))(qword_140C64A70);
}
// 140A6BDD8: using guessed type wchar_t *off_140A6BDD8[2];
// 140C63838: using guessed type __int64 (__fastcall *qword_140C63838)(_QWORD, _QWORD);
// 140C63858: using guessed type __int64 qword_140C63858;
// 140C64A70: using guessed type __int64 qword_140C64A70;
// 140C64B5C: using guessed type int dword_140C64B5C;

