//----- (00000001401F5390) ----------------------------------------------------
__int64 sub_1401F5390()
{
	if (qword_140C63838)
		return qword_140C63838(off_140A69B10, qword_140C63858);
	if (dword_140C651B0 || (int)sub_1401F5180() < 0)
		return 0i64;
	return (*(__int64(__fastcall**)(__int64))(*(_QWORD*)qword_140C65118 + 40i64))(qword_140C65118);
}
// 140A69B10: using guessed type wchar_t *off_140A69B10[2];
// 140C63838: using guessed type __int64 (__fastcall *qword_140C63838)(_QWORD, _QWORD);
// 140C63858: using guessed type __int64 qword_140C63858;
// 140C65118: using guessed type __int64 qword_140C65118;
// 140C651B0: using guessed type int dword_140C651B0;

