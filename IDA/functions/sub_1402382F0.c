#include "../winhttp.h"

//----- (00000001402382F0) ----------------------------------------------------
__int64 sub_1402382F0()
{
	if (qword_140C63838)
		return qword_140C63838(off_140A6D230, qword_140C63858);
	if (dword_140C6428C || (int)sub_1402380E0() < 0)
		return 0i64;
	return (*(__int64(__fastcall**)(__int64))(*(_QWORD*)qword_140C65328 + 40i64))(qword_140C65328);
}
// 140A6D230: using guessed type wchar_t *off_140A6D230[2];
// 140C63838: using guessed type __int64 (__fastcall *qword_140C63838)(_QWORD, _QWORD);
// 140C63858: using guessed type __int64 qword_140C63858;
// 140C6428C: using guessed type int dword_140C6428C;
// 140C65328: using guessed type __int64 qword_140C65328;

