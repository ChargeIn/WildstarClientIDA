#include "../winhttp.h"

//----- (0000000140239830) ----------------------------------------------------
__int64 sub_140239830()
{
	if (qword_140C63838)
		return qword_140C63838(off_140A6D348, qword_140C63858);
	if (dword_140C63F20 || (int)sub_140239620() < 0)
		return 0i64;
	return (*(__int64(__fastcall**)(__int64))(*(_QWORD*)qword_140C63B48 + 40i64))(qword_140C63B48);
}
// 140A6D348: using guessed type wchar_t *off_140A6D348[2];
// 140C63838: using guessed type __int64 (__fastcall *qword_140C63838)(_QWORD, _QWORD);
// 140C63858: using guessed type __int64 qword_140C63858;
// 140C63B48: using guessed type __int64 qword_140C63B48;
// 140C63F20: using guessed type int dword_140C63F20;

