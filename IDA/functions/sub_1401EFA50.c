#include "../winhttp.h"

//----- (00000001401EFA50) ----------------------------------------------------
__int64 sub_1401EFA50()
{
	if (qword_140C63838)
		return qword_140C63838(off_140A69678, qword_140C63858);
	if (dword_140C647B8 || (int)sub_1401EF840() < 0)
		return 0i64;
	return (*(__int64(__fastcall**)(__int64))(*(_QWORD*)qword_140C64450 + 40i64))(qword_140C64450);
}
// 140A69678: using guessed type wchar_t *off_140A69678[2];
// 140C63838: using guessed type __int64 (__fastcall *qword_140C63838)(_QWORD, _QWORD);
// 140C63858: using guessed type __int64 qword_140C63858;
// 140C64450: using guessed type __int64 qword_140C64450;
// 140C647B8: using guessed type int dword_140C647B8;

