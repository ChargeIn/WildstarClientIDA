#include "../winhttp.h"

//----- (0000000140226A70) ----------------------------------------------------
__int64 sub_140226A70()
{
	if (qword_140C63838)
		return qword_140C63838(off_140A6C3C0, qword_140C63858);
	if (dword_140C63DF0 || (int)sub_140226860() < 0)
		return 0i64;
	return (*(__int64(__fastcall**)(__int64))(*(_QWORD*)qword_140C63B90 + 40i64))(qword_140C63B90);
}
// 140A6C3C0: using guessed type wchar_t *off_140A6C3C0[2];
// 140C63838: using guessed type __int64 (__fastcall *qword_140C63838)(_QWORD, _QWORD);
// 140C63858: using guessed type __int64 qword_140C63858;
// 140C63B90: using guessed type __int64 qword_140C63B90;
// 140C63DF0: using guessed type int dword_140C63DF0;

