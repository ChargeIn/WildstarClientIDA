//----- (00000001402272F0) ----------------------------------------------------
__int64 sub_1402272F0()
{
	if (qword_140C63838)
		return qword_140C63838(off_140A6C430, qword_140C63858);
	if (dword_140C63A84 || (int)sub_1402270E0() < 0)
		return 0i64;
	return (*(__int64(__fastcall**)(__int64))(*(_QWORD*)qword_140C64738 + 40i64))(qword_140C64738);
}
// 140A6C430: using guessed type wchar_t *off_140A6C430[2];
// 140C63838: using guessed type __int64 (__fastcall *qword_140C63838)(_QWORD, _QWORD);
// 140C63858: using guessed type __int64 qword_140C63858;
// 140C63A84: using guessed type int dword_140C63A84;
// 140C64738: using guessed type __int64 qword_140C64738;

