#include "../winhttp.h"

//----- (00000001401F79D0) ----------------------------------------------------
__int64 sub_1401F79D0()
{
	if (qword_140C63838)
		return qword_140C63838(off_140A69D08, qword_140C63858);
	if (dword_140C65448 || (int)sub_1401F77C0() < 0)
		return 0i64;
	return (*(__int64(__fastcall**)(__int64))(*(_QWORD*)qword_140C65640 + 40i64))(qword_140C65640);
}
// 140A69D08: using guessed type wchar_t *off_140A69D08[2];
// 140C63838: using guessed type __int64 (__fastcall *qword_140C63838)(_QWORD, _QWORD);
// 140C63858: using guessed type __int64 qword_140C63858;
// 140C65448: using guessed type int dword_140C65448;
// 140C65640: using guessed type __int64 qword_140C65640;

