//----- (0000000140234BB0) ----------------------------------------------------
__int64 sub_140234BB0()
{
	if (qword_140C63838)
		return qword_140C63838(off_140A6CF58, qword_140C63858);
	if (dword_140C649A4 || (int)sub_1402349A0() < 0)
		return 0i64;
	return (*(__int64(__fastcall**)(__int64))(*(_QWORD*)qword_140C64240 + 40i64))(qword_140C64240);
}
// 140A6CF58: using guessed type wchar_t *off_140A6CF58[2];
// 140C63838: using guessed type __int64 (__fastcall *qword_140C63838)(_QWORD, _QWORD);
// 140C63858: using guessed type __int64 qword_140C63858;
// 140C64240: using guessed type __int64 qword_140C64240;
// 140C649A4: using guessed type int dword_140C649A4;

