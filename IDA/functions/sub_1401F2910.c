//----- (00000001401F2910) ----------------------------------------------------
__int64 sub_1401F2910()
{
	if (qword_140C63838)
		return qword_140C63838(off_140A698E0, qword_140C63858);
	if (dword_140C644E8 || (int)sub_1401F2700() < 0)
		return 0i64;
	return (*(__int64(__fastcall**)(__int64))(*(_QWORD*)qword_140C63BC0 + 40i64))(qword_140C63BC0);
}
// 140A698E0: using guessed type wchar_t *off_140A698E0[2];
// 140C63838: using guessed type __int64 (__fastcall *qword_140C63838)(_QWORD, _QWORD);
// 140C63858: using guessed type __int64 qword_140C63858;
// 140C63BC0: using guessed type __int64 qword_140C63BC0;
// 140C644E8: using guessed type int dword_140C644E8;

