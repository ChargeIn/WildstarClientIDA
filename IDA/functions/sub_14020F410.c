#include "../winhttp.h"

//----- (000000014020F410) ----------------------------------------------------
__int64 sub_14020F410()
{
	if (qword_140C63838)
		return qword_140C63838(off_140A6B080, qword_140C63858);
	if (dword_140C64784 || (int)sub_14020F200() < 0)
		return 0i64;
	return (*(__int64(__fastcall**)(__int64))(*(_QWORD*)qword_140C640B8 + 40i64))(qword_140C640B8);
}
// 140A6B080: using guessed type wchar_t *off_140A6B080[2];
// 140C63838: using guessed type __int64 (__fastcall *qword_140C63838)(_QWORD, _QWORD);
// 140C63858: using guessed type __int64 qword_140C63858;
// 140C640B8: using guessed type __int64 qword_140C640B8;
// 140C64784: using guessed type int dword_140C64784;

