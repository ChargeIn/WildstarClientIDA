#include "../winhttp.h"

//----- (0000000140247DB0) ----------------------------------------------------
__int64 sub_140247DB0()
{
	if (qword_140C63838)
		return qword_140C63838(off_140A6DF18, qword_140C63858);
	if (dword_140C646D4 || (int)sub_140247BA0() < 0)
		return 0i64;
	return (*(__int64(__fastcall**)(__int64))(*(_QWORD*)qword_140C63C58 + 40i64))(qword_140C63C58);
}
// 140A6DF18: using guessed type wchar_t *off_140A6DF18[2];
// 140C63838: using guessed type __int64 (__fastcall *qword_140C63838)(_QWORD, _QWORD);
// 140C63858: using guessed type __int64 qword_140C63858;
// 140C63C58: using guessed type __int64 qword_140C63C58;
// 140C646D4: using guessed type int dword_140C646D4;

