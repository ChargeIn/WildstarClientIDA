//----- (000000014021C8F0) ----------------------------------------------------
__int64 sub_14021C8F0()
{
	if (qword_140C63838)
		return qword_140C63838(off_140A6BB70, qword_140C63858);
	if (dword_140C6405C || (int)sub_14021C6E0() < 0)
		return 0i64;
	return (*(__int64(__fastcall**)(__int64))(*(_QWORD*)qword_140C65408 + 40i64))(qword_140C65408);
}
// 140A6BB70: using guessed type wchar_t *off_140A6BB70[2];
// 140C63838: using guessed type __int64 (__fastcall *qword_140C63838)(_QWORD, _QWORD);
// 140C63858: using guessed type __int64 qword_140C63858;
// 140C6405C: using guessed type int dword_140C6405C;
// 140C65408: using guessed type __int64 qword_140C65408;

