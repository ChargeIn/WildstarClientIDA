//----- (00000001402195F0) ----------------------------------------------------
__int64 sub_1402195F0()
{
	if (qword_140C63838)
		return qword_140C63838(off_140A6B8D0, qword_140C63858);
	if (dword_140C64DB0 || (int)sub_1402193E0() < 0)
		return 0i64;
	return (*(__int64(__fastcall**)(__int64))(*(_QWORD*)qword_140C65100 + 40i64))(qword_140C65100);
}
// 140A6B8D0: using guessed type wchar_t *off_140A6B8D0[2];
// 140C63838: using guessed type __int64 (__fastcall *qword_140C63838)(_QWORD, _QWORD);
// 140C63858: using guessed type __int64 qword_140C63858;
// 140C64DB0: using guessed type int dword_140C64DB0;
// 140C65100: using guessed type __int64 qword_140C65100;

