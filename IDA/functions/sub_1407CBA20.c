#include "../winhttp.h"

//----- (00000001407CBA20) ----------------------------------------------------
__int64 __fastcall sub_1407CBA20(__int64 a1)
{
	__int64 v2; // r8

	(*(void(__fastcall**)(__int64))(*(_QWORD*)qword_140C65688 + 192i64))(qword_140C65688);
	(*(void(__fastcall**)(__int64, _QWORD, _QWORD, _QWORD, _QWORD))(*(_QWORD*)qword_140C65670 + 352i64))(
		qword_140C65670,
		0i64,
		*(_QWORD*)(a1 + 6576),
		0i64,
		0i64);
	(*(void(__fastcall**)(__int64, _QWORD, __int64))(*(_QWORD*)qword_140C65670 + 376i64))(
		qword_140C65670,
		*(_QWORD*)(a1 + 5744),
		1i64);
	return (*(__int64(__fastcall**)(__int64, __int64, __int64, _QWORD, _QWORD, _QWORD))(*(_QWORD*)qword_140C65670
		+ 664i64))(
			qword_140C65670,
			0xFFFFFFFFi64,
			v2,
			0i64,
			0i64,
			0i64);
}
// 1407CBAA5: variable 'v2' is possibly undefined
// 140C65670: using guessed type __int64 qword_140C65670;
// 140C65688: using guessed type __int64 qword_140C65688;

