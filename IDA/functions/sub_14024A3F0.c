#include "../winhttp.h"

//----- (000000014024A3F0) ----------------------------------------------------
__int64 sub_14024A3F0()
{
	if (qword_140C63838)
		return qword_140C63838(off_140A6E110, qword_140C63858);
	if (dword_140C64BC0 || (int)sub_14024A1E0() < 0)
		return 0i64;
	return (*(__int64(__fastcall**)(__int64))(*(_QWORD*)qword_140C65460 + 40i64))(qword_140C65460);
}
// 140A6E110: using guessed type wchar_t *off_140A6E110[2];
// 140C63838: using guessed type __int64 (__fastcall *qword_140C63838)(_QWORD, _QWORD);
// 140C63858: using guessed type __int64 qword_140C63858;
// 140C64BC0: using guessed type int dword_140C64BC0;
// 140C65460: using guessed type __int64 qword_140C65460;

