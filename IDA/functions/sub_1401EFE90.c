//----- (00000001401EFE90) ----------------------------------------------------
__int64 sub_1401EFE90()
{
	if (qword_140C63838)
		return qword_140C63838(off_140A696B0, qword_140C63858);
	if (dword_140C64324 || (int)sub_1401EFC80() < 0)
		return 0i64;
	return (*(__int64(__fastcall**)(__int64))(*(_QWORD*)qword_140C63F68 + 40i64))(qword_140C63F68);
}
// 140A696B0: using guessed type wchar_t *off_140A696B0[2];
// 140C63838: using guessed type __int64 (__fastcall *qword_140C63838)(_QWORD, _QWORD);
// 140C63858: using guessed type __int64 qword_140C63858;
// 140C63F68: using guessed type __int64 qword_140C63F68;
// 140C64324: using guessed type int dword_140C64324;

