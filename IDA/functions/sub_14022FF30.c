//----- (000000014022FF30) ----------------------------------------------------
__int64 sub_14022FF30()
{
	if (qword_140C63838)
		return qword_140C63838(off_140A6CB68, qword_140C63858);
	if (dword_140C63898 || (int)sub_14022FD20() < 0)
		return 0i64;
	return (*(__int64(__fastcall**)(__int64))(*(_QWORD*)qword_140C64480 + 40i64))(qword_140C64480);
}
// 140A6CB68: using guessed type wchar_t *off_140A6CB68[2];
// 140C63838: using guessed type __int64 (__fastcall *qword_140C63838)(_QWORD, _QWORD);
// 140C63858: using guessed type __int64 qword_140C63858;
// 140C63898: using guessed type int dword_140C63898;
// 140C64480: using guessed type __int64 qword_140C64480;

