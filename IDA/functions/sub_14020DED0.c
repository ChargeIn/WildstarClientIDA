//----- (000000014020DED0) ----------------------------------------------------
__int64 sub_14020DED0()
{
	if (qword_140C63838)
		return qword_140C63838(off_140A6AF68, qword_140C63858);
	if (dword_140C655A0 || (int)sub_14020DCC0() < 0)
		return 0i64;
	return (*(__int64(__fastcall**)(__int64))(*(_QWORD*)qword_140C63E18 + 40i64))(qword_140C63E18);
}
// 140A6AF68: using guessed type wchar_t *off_140A6AF68[2];
// 140C63838: using guessed type __int64 (__fastcall *qword_140C63838)(_QWORD, _QWORD);
// 140C63858: using guessed type __int64 qword_140C63858;
// 140C63E18: using guessed type __int64 qword_140C63E18;
// 140C655A0: using guessed type int dword_140C655A0;

