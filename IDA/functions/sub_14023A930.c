#include "../winhttp.h"

//----- (000000014023A930) ----------------------------------------------------
__int64 sub_14023A930()
{
	if (qword_140C63838)
		return qword_140C63838(off_140A6D428, qword_140C63858);
	if (dword_140C63AB0 || (int)sub_14023A720() < 0)
		return 0i64;
	return (*(__int64(__fastcall**)(__int64))(*(_QWORD*)qword_140C64198 + 40i64))(qword_140C64198);
}
// 140A6D428: using guessed type wchar_t *off_140A6D428[2];
// 140C63838: using guessed type __int64 (__fastcall *qword_140C63838)(_QWORD, _QWORD);
// 140C63858: using guessed type __int64 qword_140C63858;
// 140C63AB0: using guessed type int dword_140C63AB0;
// 140C64198: using guessed type __int64 qword_140C64198;

